...
interface ServiceRecord {
    noSurat: string; kategori: "AR"|"PCO"; jenisLayanan: string; tipeKontrak: string;
    tanggal: Date; status: string; marketingNama: string; garansiTahun?: number; jenisGaransi?: string;
}
...
const STATUS_COLOR: Record<string,string> = {
    ...
    sent_to_client:"bg-yellow-100 text-yellow-700", deal:"bg-emerald-100 text-emerald-700", cancelled:"bg-slate-100 text-slate-400",
};
const STATUS_LABEL: Record<string,string> = {
    draft:"Draft", pending:"Menunggu", approved:"Disetujui", rejected:"Ditolak",
    sent_to_client:"Dikirim", deal:"Deal", cancelled:"Batal",
};
...
function ChecklistModal({ customer, onClose, onSaved }: { customer: DerivedCustomer; onClose: ()=>void; onSaved: (c: ControlChecklist)=>void }) {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            await setDoc(doc(db, "customerChecklists", id), {
                ...
                catatan: entry.catatan, hasilKontrol: entry.hasilKontrol, createdAt: Timestamp.fromDate(new Date()),
            });
            ...
            <div className="relative bg-white rounded-2xl w-full max-w-md shadow-2xl p-6 space-y-4">
                ...
                <div className="space-y-3">
                    <div>
                        ...
                        <input type="date" value={tanggal} onChange={e=>setTanggal(e.target.value)} className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-300"/>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <textarea rows={3} value={catatan} onChange={e=>setCatatan(e.target.value)} placeholder="Kondisi lapangan, temuan, tindakan..." className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-300 resize-none"/>
                    </div>
                </div>
                <div className="flex gap-2">
                    ...
                    <button onClick={handleSave} disabled={saving} className="flex-1 py-2.5 text-sm rounded-xl bg-blue-600 text-white font-semibold hover:bg-blue-700 disabled:opacity-50 flex items-center justify-center gap-2">
...
function CustomerCard({ customer, expanded, onToggle, onAddChecklist }: { customer: DerivedCustomer; expanded: boolean; onToggle: ()=>void; onAddChecklist: ()=>void }) {
        ...
        <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
            <button onClick={onToggle} className="w-full px-5 py-4 flex items-start gap-4 text-left hover:bg-slate-50 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    {customer.address && <p className="text-xs text-slate-400 truncate flex items-center gap-1 mb-0.5"><MapPin size={10} className="shrink-0"/> {customer.address}</p>}
                    {customer.wa && (
                        <a href={`https://wa.me/${customer.wa.replace(/^0/,"62").replace(/\D/g,"")}`} target="_blank" rel="noopener noreferrer" onClick={e=>e.stopPropagation()} className="inline-flex items-center gap-1 text-[10px] text-green-600 hover:text-green-700 font-medium mb-1">
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap mt-1">
                        ...
                        {lastCl && (
                            <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full flex items-center gap-0.5 ${lastCl.hasilKontrol==="baik"?"bg-emerald-100 text-emerald-700":lastCl.hasilKontrol==="darurat"?"bg-red-100 text-red-700":"bg-amber-100 text-amber-700"}`}>
            ...
            {expanded && (
                <div className="border-t border-slate-100">
                    ...
                    <div className="px-5 py-4 bg-slate-50 border-b border-slate-100 space-y-2">
                        <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2 flex items-center gap-1"><Building2 size={10}/> Informasi Klien</p>
                        ...
                        {customer.up && (
                            <div className="flex items-center gap-2 text-xs"><Phone size={12} className="text-slate-400 shrink-0"/><span className="text-slate-700">U.p. {customer.up}</span></div>
                        )}
                        {customer.wa && (
                            <div className="flex items-center gap-2 text-xs">
                                <MessageCircle size={12} className="text-green-500 shrink-0"/>
                                <a href={`https://wa.me/${customer.wa.replace(/^0/,"62").replace(/\D/g,"")}`} target="_blank" rel="noopener noreferrer" className="text-green-600 hover:underline font-medium">{customer.wa}</a>
                            </div>
                        )}
                        {customer.lastServiceDate && (
                            <div className="flex items-center gap-2 text-xs"><Wrench size={12} className="text-slate-400 shrink-0"/><span className="text-slate-600">Terakhir dikerjakan: <strong>{formatDate(customer.lastServiceDate)}</strong></span></div>
                        )}
                    </div>
                    ...
                    {customer.warranties.length > 0 && (
                        <div className="px-5 py-4 bg-purple-50 border-b border-purple-100">
                            <p className="text-[10px] font-bold uppercase tracking-widest text-purple-500 mb-3 flex items-center gap-1"><Shield size={10}/> Status Garansi</p>
                                ...
                                {customer.warranties.map(w=>(
                                    <div key={w.quotationId} className={`p-3 rounded-xl border text-xs ${w.isActive?"bg-white border-purple-200":"bg-slate-50 border-slate-200 opacity-60"}`}>
                                        <div className="flex items-center justify-between mb-1">
                                            ...
                                            {w.isActive ? <span className="font-bold text-emerald-600">{w.daysRemaining} hari lagi</span> : <span className="text-slate-400">Kadaluarsa</span>}
                                        </div>
                    ...
                    <div className="px-5 py-4 border-b border-slate-100">
                        <div className="flex items-center justify-between mb-3">
                            <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 flex items-center gap-1"><ClipboardCheck size={10}/> Kontrol Berkala</p>
                            <button onClick={e=>{e.stopPropagation();onAddChecklist();}} className="text-xs font-semibold text-blue-600 hover:text-blue-700 bg-blue-50 px-2.5 py-1 rounded-lg border border-blue-200">+ Tambah</button>
                        </div>
                        {customer.checklists.length === 0 ? (
                            ...
                            <div className="space-y-2 max-h-44 overflow-y-auto">
                                {[...customer.checklists].sort((a,b)=>b.tanggal.getTime()-a.tanggal.getTime()).map(c=>(
                                    <div key={c.id} className={`flex items-start gap-2.5 p-2.5 rounded-xl border text-xs ${c.hasilKontrol==="baik"?"bg-emerald-50 border-emerald-100":c.hasilKontrol==="darurat"?"bg-red-50 border-red-100":"bg-amber-50 border-amber-100"}`}>
                    ...
                    <div className="px-5 py-4">
                        <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-3 flex items-center gap-1"><FileText size={10}/> Riwayat Pekerjaan</p>
                        <div className="space-y-2">
                            {customer.services.sort((a,b)=>b.tanggal.getTime()-a.tanggal.getTime()).map(s=>(
                                <div key={s.noSurat} className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                                    <div className="flex items-center gap-2 flex-wrap mb-1">
                                        <code className={`text-xs font-bold font-mono px-1.5 py-0.5 rounded ${s.kategori==="AR"?"bg-purple-100 text-purple-700":"bg-cyan-100 text-cyan-700"}`}>{s.noSurat}</code>
                                        <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${STATUS_COLOR[s.status]??"bg-slate-100 text-slate-500"}`}>{STATUS_LABEL[s.status]??s.status}</span>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                                        ...
                                        {s.garansiTahun && s.garansiTahun > 0 && (
                                            <span className="text-[10px] text-purple-600 font-semibold">· Garansi {s.garansiTahun}thn ({s.jenisGaransi??"Anti Rayap"})</span>
                                        )}
                                    </div>
                                </div>
                            ))}
...
export function CustomersPage() {
    ...
    const customerMap = useMemo(() => {
        ...
        quotations.forEach(q => {
            ...
            if (!map.has(key)) map.set(key, { id:key, name:q.kepadaNama, address:q.kepadaAlamatLines?.[0]??"", addressFull:q.kepadaAlamatLines??[], up:q.kepadaUp, wa:q.kepadaWa??"", services:[], totalQuotations:0, totalDeal:0, lastServiceDate:null, jasaDigunakan:[], warranties:[], activeWarranty:null, checklists:[] });
            const c = map.get(key)!;
            c.totalQuotations++;
            const jasa = LAYANAN_CONFIG[q.jenisLayanan]?.label.split("—")[1]?.trim() ?? q.jenisLayanan;
            c.services.push({ noSurat:q.noSurat, kategori:q.kategori, jenisLayanan:jasa, tipeKontrak:q.tipeKontrak, tanggal:q.tanggal, status:q.status, marketingNama:q.marketingNama, garansiTahun:q.garansiTahun, jenisGaransi:q.jenisGaransi });
            if (!c.jasaDigunakan.includes(jasa)) c.jasaDigunakan.push(jasa);
            if (q.status === "deal") {
                c.totalDeal++;
                if (q.dealAt && (!c.lastServiceDate || q.dealAt > c.lastServiceDate)) c.lastServiceDate = q.dealAt;
            ...
            if (!c.address && q.kepadaAlamatLines?.[0]) { c.address = q.kepadaAlamatLines[0]; c.addressFull = q.kepadaAlamatLines; }
    ...
    if (debouncedSearch) {
        ...
        customers = customers.filter(c => c.name.toLowerCase().includes(s) || c.address.toLowerCase().includes(s) || c.jasaDigunakan.some(j=>j.toLowerCase().includes(s)));
    }
    customers.sort((a,b) => sortBy==="name" ? a.name.localeCompare(b.name) : sortBy==="deal" ? b.totalDeal-a.totalDeal : (b.services[0]?.tanggal?.getTime()??0)-(a.services[0]?.tanggal?.getTime()??0));
    ...
    return (
        <div className="p-4 md:p-6 max-w-screen-lg mx-auto space-y-5">
            <div>
                ...
                <p className="text-sm text-slate-500 mt-0.5">{canSeeAll ? "Database klien perusahaan" : "Klien dari quotation kamu"}</p>
            </div>
            ...
            <div className="flex gap-2 flex-wrap">
                <div className="flex items-center gap-2 flex-1 min-w-0 bg-white border border-slate-200 rounded-xl px-3 py-2 focus-within:ring-2 focus-within:ring-blue-100">
                    ...
                    <input className="flex-1 text-sm bg-transparent outline-none text-slate-800 placeholder:text-slate-400 min-w-0" placeholder="Cari nama, alamat, atau jenis jasa..." value={searchQ} onChange={e=>setSearchQ(e.target.value)}/>
                    {searchQ && <button onClick={()=>setSearchQ("")} className="text-slate-400 hover:text-slate-600 shrink-0"><X size={13}/></button>}
                </div>
                <button onClick={()=>setFilterWarranty(v=>!v)} className={`px-3 py-2 text-xs font-semibold rounded-xl border transition-colors ${filterWarranty?"bg-purple-600 text-white border-purple-600":"bg-white text-slate-600 border-slate-200 hover:bg-slate-50"}`}>🛡 Bergaransi</button>
            ...
            {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-16 text-slate-400 text-center">
                    ...
                    <p className="font-medium text-slate-500">{debouncedSearch||filterWarranty?"Klien tidak ditemukan":"Belum ada data klien"}</p>
                    <p className="text-sm mt-1">{debouncedSearch||filterWarranty?"Coba ubah filter.":"Data muncul otomatis dari quotation."}</p>
                </div>
            ) : (
                <div className="space-y-3">
                    {customers.map(c=>(
                        <CustomerCard key={c.id} customer={c} expanded={expandedId===c.id} onToggle={()=>setExpandedId(prev=>prev===c.id?null:c.id)} onAddChecklist={()=>setChecklistTarget(c)}/>
                    ))}
