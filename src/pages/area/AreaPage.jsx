...
function Step2({ nama, alamatLines, up, wa, knownCustomers, onNama, onAlamat, onUp, onWa, errors }: {
    ...
    const suggestions = nama.length >= 2
        ? knownCustomers.filter(c => c.name.toLowerCase().includes(nama.toLowerCase())).slice(0, 5)
        : [];
            ...
            <Field label="Nama Klien / Perusahaan" required error={errors.nama}>
                <div className="relative">
                    <input className={inputCls} value={nama}
                        onChange={e => { onNama(e.target.value); setFilledFrom(null); setShowSuggestions(true); }}
                        onFocus={() => setShowSuggestions(true)}
                        onBlur={() => setTimeout(() => setShowSuggestions(false), 150)}
                        placeholder="PT Contoh Indonesia / Bapak Ahmad..." />
                    {showSuggestions && suggestions.length > 0 && (
                        <div className="absolute top-full left-0 right-0 z-50 mt-1 bg-white border border-blue-200 rounded-xl shadow-lg overflow-hidden">
                            <p className="px-3 py-1.5 text-[10px] font-bold uppercase tracking-wide text-slate-400 bg-blue-50 border-b border-blue-100">
                                Pelanggan sebelumnya — klik untuk isi otomatis
                            </p>
                            {suggestions.map((c, i) => (
                                <button key={i} type="button"
                                    ...
                                    {c.alamatLines[0] && <p className="text-xs text-slate-400 truncate">{c.alamatLines[0]}</p>}
                                    {c.wa && <p className="text-xs text-green-600">📱 {c.wa}</p>}
                                </button>
                            ))}
                        </div>
                    )}
...
export function QuotationFormPage() {
    ...
    useEffect(() => {
        ...
        getQuotations({ companyId: user.companyId }).then(quotes => {
            ...
            quotes.filter(q => q.status === "deal" || q.status === "approved").forEach(q => {
